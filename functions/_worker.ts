...
interface Env {
  ...
  MAINTENANCE_IMAGES: R2Bucket
  MAINTENANCE_INVOICES: R2Bucket
  ACCESS_CODE?: string
  ASSETS: Fetcher
}
...
app.get('/api/reconciliation/summary', async (c) => {
  const { results } = await c.env.DB.prepare(
    "SELECT ledger_type, status, count(*) as count, round(sum(ledger_amount),2) as total_ledger, round(sum(bank_amount),2) as total_bank, round(sum(variance),2) as total_variance FROM reconciliation GROUP BY ledger_type, status"
  ).all()
...
app.post('/api/work-orders', async (c) => {
  ...
  ).bind(id, body.property_id, body.unit_id || null, body.contractor_id || null, body.description, body.status || 'open', body.urgency || 'routine', body.liability || null, body.cost_estimate || 0).run()
  return c.json({ id }, 201)
})
...
app.post('/api/pl', async (c) => {
  ...
  const existing = await c.env.DB.prepare(
    'SELECT id FROM pl_data WHERE property_id = ? AND year = ? AND category = ?'
...
// ── Maintenance Tracker ──────────────────────────────────
// GET: List maintenance items from rental_ledger (where category = 'maintenance' or debit > 0)
app.get('/api/maintenance', async (c) => {
  ...
  const clauses: string[] = [`property_id NOT IN (${HIDDEN_PROPS_SQL})`]
  const params: any[] = [...HIDDEN_PROPERTIES]
  
  if (p && p !== 'all') { clauses.push('property_id = ?'); params.push(p) }
  if (status && status !== 'all') { clauses.push('category = ?'); params.push(status) }
  
  const where = clauses.length ? ' WHERE ' + clauses.join(' AND ') : ''

  const countRow = await c.env.DB.prepare(
    `SELECT count(*) as total FROM rental_ledger${where}`
  ).bind(...params).first() as any

  const { results } = await c.env.DB.prepare(
    `SELECT r.*, p.name as property_name 
     FROM rental_ledger r 
     LEFT JOIN properties p ON p.id = r.property_id${where} 
...
// GET: Single maintenance item with images
app.get('/api/maintenance/:id', async (c) => {
  const id = c.req.param('id')
  ...
  // Get images from R2
  const images = await c.env.MAINTENANCE_IMAGES.list({ prefix: `${id}/` })
  const invoices = await c.env.MAINTENANCE_INVOICES.list({ prefix: `${id}/` })

  return c.json({ ...item as any, images: images.objects, invoices: invoices.objects })
})

// POST: Upload image for maintenance item
app.post('/api/maintenance/:id/images', async (c) => {
  const id = c.req.param('id')
  ...
  await c.env.MAINTENANCE_IMAGES.put(filename, body, {
    httpMetadata: { contentType },
  })

  logActivity(c, 'upload', 'maintenance-image', id, filename)
  return c.json({ filename }, 201)
})

// DELETE: Remove image from maintenance item
app.delete('/api/maintenance/:id/images/:filename', async (c) => {
  const id = c.req.param('id')
  const filename = c.req.param('filename')
  const key = `${id}/${filename}`
  
  await c.env.MAINTENANCE_IMAGES.delete(key)
  logActivity(c, 'delete', 'maintenance-image', id, key)
  return c.json({ ok: true })
})

// POST: Upload invoice for maintenance item
app.post('/api/maintenance/:id/invoices', async (c) => {
  const id = c.req.param('id')
  ...
  await c.env.MAINTENANCE_INVOICES.put(filename, body, {
    httpMetadata: { contentType },
  })

  logActivity(c, 'upload', 'maintenance-invoice', id, filename)
  return c.json({ filename }, 201)
})

// DELETE: Remove invoice from maintenance item
app.delete('/api/maintenance/:id/invoices/:filename', async (c) => {
  const id = c.req.param('id')
  const filename = c.req.param('filename')
  const key = `${id}/${filename}`
  
  await c.env.MAINTENANCE_INVOICES.delete(key)
  logActivity(c, 'delete', 'maintenance-invoice', id, key)
  return c.json({ ok: true })
})

// GET: Get pre-signed URL for R2 object
app.get('/api/maintenance/:id/images/:filename/url', async (c) => {
  const id = c.req.param('id')
  const filename = c.req.param('filename')
  const key = `${id}/${filename}`
  
  const object = await c.env.MAINTENANCE_IMAGES.get(key)
  if (!object) return c.json({ error: 'not found' }, 404)
  
  const arrayBuffer = await object.arrayBuffer()
  const base64 = btoa(String.fromCharCode(...new Uint8Array(arrayBuffer)))
  const dataUrl = `data:${object.httpMetadata?.contentType || 'image/jpeg'};base64,${base64}`
